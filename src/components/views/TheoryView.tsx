...
export function TheoryView({ session }: { session: Session }) {
  ...
  if (!data) return <div className="p-8 text-center text-slate-500">{t(language, 'theoryPreparing')}</div>;
    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-4 animate-fade-in pb-4 w-full">
      <div className="bg-slate-900 p-4 rounded-lg border border-slate-800 flex flex-col gap-3">
        <h2 className="text-sm font-bold flex items-center gap-2 text-white uppercase tracking-widest">
          <Clock size={16} className="text-blue-500" /> {t(language, 'sessionOverview')}
        </h2>
          ...
          <div className="flex gap-4">
            ...
            <ul className="list-disc pl-4 space-y-1 text-slate-300">
              {session.objectives.length > 0 ? (
                ...
                <li>{translateContent('세부 학습 목표가 곧 제공됩니다.', language)}</li>
              )}
            </ul>
          </div>
          {session.ncsUnit && (
             <div className="flex gap-4"><span className="text-slate-500 w-16 shrink-0 block">{t(language, 'ncsUnit')}</span> <span className="text-slate-300">{translateContent(session.ncsUnit, language)}</span></div>
          )}
          <div className="flex gap-4"><span className="text-slate-500 w-16 shrink-0 block">{t(language, 'operatingRhythm')}</span> <span className="text-slate-300">{translateContent(session.rhythmLabel || t(language, 'standardSession'), language)} · {t(language, 'sessionTiming')}</span></div>
        </div>
      </div>
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 bg-slate-900 p-4 rounded-lg border border-slate-800 relative overflow-hidden flex flex-col gap-3">
            <h3 className="text-xs font-bold text-amber-500 uppercase tracking-widest flex items-center gap-2"><Lightbulb size={14}/> {t(language, 'introReview')}</h3>
            <div className="space-y-2">
              <p className="text-[10px] text-slate-500">{t(language, 'learnerQuestion')}</p>
              {data.intro.questions.map((qObj, i) => (
                ...
                >
                  <div className="flex justify-between items-center gap-4">
                      <div className="font-bold text-slate-200"><span className="text-blue-500 mr-2">Q.</span>{translateContent(qObj.q, language)}</div>
                  ...
                  {expandedQId === i && (
                      <div className="mt-3 pt-3 border-t border-slate-800/80 text-emerald-400/90 leading-relaxed font-medium animate-fade-in">
                          <span className="text-emerald-500 font-bold mr-2">A.</span>{translateContent(qObj.a, language)}
                      </div>
                  )}
                </div>
              ))}
            ...
            {data.mainContent.map((content, idx) => (
                <div key={idx} className="bg-slate-900 p-4 rounded-lg border border-slate-800 flex flex-col gap-3">
                    <div className="flex justify-between items-center">
                        <h3 className="text-sm font-bold text-white">{idx+1}. {translateContent(content.title, language)}</h3>
                    ...
                    <div className="text-slate-300 text-xs leading-relaxed p-3 bg-slate-950 rounded border border-slate-800/80">
                        {translateContent(content.content, language)}
                    </div>
                </div>
            ))}

            <div className="bg-amber-900/10 p-4 rounded-lg border border-amber-500/20 flex flex-col gap-3">
                 <h3 className="text-xs font-bold text-amber-500 uppercase tracking-widest flex items-center gap-2"><AlertTriangle size={14}/> {t(language, 'fieldMisconceptions')}</h3>
                    ...
                    {data.misconceptions.map((mc, idx) => (
                        <div key={idx} className="p-3 bg-slate-950/80 rounded border border-slate-800 text-xs">
                            <div className="flex items-start gap-2 mb-1.5">
                                <span className="bg-red-900/30 text-red-500 border border-red-500/30 px-1.5 py-0.5 rounded text-[9px] font-bold shrink-0 mt-0.5 font-mono">{t(language, 'misconception')}</span>
                                <span className="text-slate-400 line-through decoration-red-900/50">{translateContent(mc.wrong, language)}</span>
                            </div>
                            <div className="flex items-start gap-2">
                                <span className="bg-green-900/30 text-green-500 border border-green-500/30 px-1.5 py-0.5 rounded text-[9px] font-bold shrink-0 mt-0.5 font-mono">{t(language, 'correction')}</span>
                                <span className="text-slate-200">{translateContent(mc.correction, language)}</span>
                            </div>
                        </div>
                    ))}
                 </div>
      ...
      <div className="flex gap-4">
         ...
         <div className="flex-1 bg-slate-900 p-4 rounded-lg border border-slate-800 relative overflow-hidden flex flex-col gap-3">
             <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"><Target size={14}/> {t(language, 'summaryCheck')}</h3>
             <div className="bg-slate-950 p-3 rounded text-xs text-slate-300 border border-slate-800 space-y-1">
                ...
                <p>{translateContent(data.summary.nextSessionLink, language)}</p>
             </div>
         </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {data.educatorNotes && (
            <div className="bg-slate-900 p-4 rounded-lg border border-slate-800">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-3"><FileText size={14}/> {t(language, 'educatorNotes')}</h3>
              <p className="text-xs text-slate-300 leading-relaxed">{translateContent(data.educatorNotes, language)}</p>
            </div>
          )}
          {data.studentSlides && (
            <div className="bg-slate-900 p-4 rounded-lg border border-slate-800">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-3"><FileText size={14}/> {t(language, 'studentSlides')}</h3>
              <pre className="text-xs text-slate-300 leading-relaxed whitespace-pre-wrap font-sans">{translateContent(data.studentSlides, language)}</pre>
            </div>
          )}
        </div>
      )}
