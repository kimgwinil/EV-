...
import { t, translateContent, translateSessionTitle, translateTheme, useLanguageStore } from '../../i18n';
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 p-8 flex items-center justify-center animate-fade-in backdrop-blur-sm">
           <div className="bg-slate-900 border border-slate-800 rounded-lg shadow-2xl max-w-lg w-full overflow-hidden flex flex-col">
              <div className="bg-amber-950/50 border-b border-amber-900/50 p-4 flex justify-between items-center text-amber-500">
                 <h2 className="font-bold flex items-center gap-2 uppercase tracking-widest text-sm"><ShieldAlert size={18}/> {translateContent('현장 안전 지침서 (LOTO)', language)}</h2>
                 <button onClick={() => setIsManualOpen(false)} className="hover:text-amber-300 transition-colors"><X size={20}/></button>
              </div>
              <div className="p-6 space-y-4 text-sm text-slate-300">
                  <p className="text-amber-400/80 text-xs bg-amber-950/20 p-3 rounded font-mono border border-amber-900/30">{translateContent('WARNING: 300V 이상 고전압 시스템 작업 시 아래 절차를 타협하지 마십시오. 생명과 직결됩니다.', language)}</p>
                  ...
                  <div className="space-y-3 font-medium">
                      <div className="flex gap-3 items-start">
                         ...
                         <div>
                            <div className="text-slate-200">{translateContent('개인보호장구(PPE) 착용 및 점검', language)}</div>
                            <div className="text-xs text-slate-500 font-normal mt-0.5">{translateContent('절연 장갑의 핀홀 검사(공기 주입 후 새는 곳 확인)를 수행하고 절연복, 절연화 착용', language)}</div>
                         </div>
                      </div>
                      <div className="flex gap-3 items-start">
                         ...
                         <div>
                            <div className="text-slate-200">{translateContent('차량 시스템(IG) 전원 OFF 및 12V 탈거', language)}</div>
                            <div className="text-xs text-slate-500 font-normal mt-0.5">{translateContent('시동을 완전 차단하고, 12V (-) 보조배터리 단자를 탈거하여 릴레이 오작동 방지', language)}</div>
                         </div>
                      </div>
                      <div className="flex gap-3 items-start">
                         ...
                         <div>
                            <div className="text-slate-200">{translateContent('고전압 서비스 플러그(MSD) 탈거', language)}</div>
                            <div className="text-xs text-slate-500 font-normal mt-0.5">{translateContent('안전 플러그를 해제하고, 타인이 장착할 수 없도록 반드시 작업자 본인 주머니에 보관', language)}</div>
                         </div>
                      </div>
                      <div className="flex gap-3 items-start">
                         ...
                         <div>
                            <div className="text-slate-200">{translateContent('평활 커패시터 자연 방전 대기 (최소 5분)', language)}</div>
                            <div className="text-xs text-slate-500 font-normal mt-0.5">{translateContent('차단 후에도 인버터 내부 커패시터에 수백 볼트의 치명적 잔류 전압이 남아있으므로 대기', language)}</div>
                         </div>
                      </div>
                      <div className="flex gap-3 items-start">
                         ...
                         <div>
                            <div className="text-red-400 font-bold">{translateContent('인버터 전압 측정 및 0V 검출 확인 (검전)', language)}</div>
                            <div className="text-xs text-slate-400 font-normal mt-0.5">{translateContent('가장 중요한 최종 단계. CAT III 이상의 멀티미터로 실 고전압 단자를 측정하여 0V(또는 30V 이하의 극소전압)임을 눈으로 확인한 뒤 본 작업에 착수', language)}</div>
                         </div>
                      </div>
                  </div>
              </div>
              <div className="p-4 bg-slate-950/50 border-t border-slate-800 flex justify-end">
                 <button onClick={() => setIsManualOpen(false)} className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs rounded transition-colors uppercase tracking-widest">{translateContent('인지 및 확인', language)}</button>
              </div>
           </div>
        </div>
      )}
